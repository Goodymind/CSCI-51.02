...
struct IntList
{
    ...
    {
        if (first == NULL)                                  // checks if first node exists
        {
            IntNode* node = this->createFirstNode(value);   // creates first node if not
            return node;
        }

        IntNode* node = this->insertAfter(last, value);     // inserts after last node
        last = node;
        return node;
    }
